...
Run backend checks with `cd apps/api && python -m pytest`. Run frontend checks with `cd apps/web && npm run lint && npm run build`. The API creates tables during startup in Phase 0; migrations can be introduced before production deployment.
